...
class Create_Account:
    def __init__(self,account_type,wanted_ou,groups):
        ...
        with open(self.awk_file, mode='w') as self.file:
            self.inject_org = subprocess.Popen(["sed","-e",self.sed_params,self.temp_file], stdout=self.file)
        ...
        self.gam(self.account_type,wanted_ou,groups,self.awk_file)

    def copy_file(self,temp_file,awk_file):
        cp = subprocess.Popen(["cp",temp_file,awk_file], stdout=subprocess.PIPE)
        cp.wait()

    def gam(self,account_type,wanted_ou,groups,awk_file):
        self.account_type = account_type
        ...
        if yn == "n":
            self.__init__(account_type,wanted_ou,groups)
        else:
            try:
